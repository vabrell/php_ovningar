...
SELECT category, SUM(specimens) AS specimens
FROM animals
GROUP BY category
...
SELECT COUNT(id) AS species FROM animals WHERE category = 'däggdjur';

-- Skriv ut en lista över antalet djurarter per kategori.
SELECT category, COUNT(id) AS species FROM animals GROUP BY category;
...
SELECT category, COUNT(id) AS three_to_ten_specimens FROM animals WHERE specimens BETWEEN 3 AND 10 GROUP BY category;